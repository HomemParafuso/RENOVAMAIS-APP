#!/usr/bin/env node
// Script para testar o login de uma geradora na interface do usuário
// @ts-check
...
// Função para testar o login de uma geradora na interface do usuário
async function testGeradoraLoginUI(email, password) {
...
async function main() {
  ...
  if (!email || !password) {
    console.error('Uso: node test-geradora-login-ui.js <email> <senha>');
    process.exit(1);
  }

  // Executar o teste
  try {
    const success = await testGeradoraLoginUI(email, password);
