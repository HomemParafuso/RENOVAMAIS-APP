#!/usr/bin/env node
// Script para testar o login de uma geradora usando o navegador
// @ts-check
...
// Função para testar o login de uma geradora usando o navegador
async function testGeradoraLoginBrowser(email, password) {
  console.log(`Iniciando teste de login para geradora: ${email}`);
  ...
  try {
    ...
    console.log('Screenshot da página de login salvo como login-page.png');
    ...
    console.log('Screenshot do formulário preenchido salvo como login-form-filled.png');
    ...
    console.log('Screenshot após o login salvo como after-login.png');
    
    // Verificar se o login foi bem-sucedido
    const url = page.url();
    console.log('URL atual:', url);
    
    if (url.includes('/dashboard') || url.includes('/geradora') || url.includes('/admin')) {
...
async function main() {
  ...
  if (!email || !password) {
    console.error('Uso: node test-geradora-login-browser.js <email> <senha>');
    process.exit(1);
  }

  // Executar o teste
  try {
    const success = await testGeradoraLoginBrowser(email, password);
